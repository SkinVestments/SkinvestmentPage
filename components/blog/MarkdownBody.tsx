...
const INTERNAL_ORIGIN = 'https://skinvestments.app';

function hrefToInternalPath(href: string | undefined): string | null {
  if (!href) return null;
  if (href.startsWith('/') && !href.startsWith('//')) return href;
  if (href.startsWith(INTERNAL_ORIGIN)) {
    const path = href.slice(INTERNAL_ORIGIN.length) || '/';
  ...
  >
    <ReactMarkdown
      ...
      components={{
        ...
        strong: ({ children }) => (
          <strong className="font-semibold text-steam-text">{children}</strong>
        ),
        a: ({ href, children }) => {
